...
const Sidebar = () => {
  ...
  const { otherUserStatuses, openStory } = useStatusStore();
  const [searchParams, setSearchParams] = useSearchParams();
  const [mainTab, setMainTab] = useState("chats"); // 'chats' | 'status'
  ...
  // Sync tab and filter tab when navigated from mobile bottom navigation or link
  useEffect(() => {
    const tab = searchParams.get("tab");
    if (tab === "status" || tab === "chats") {
      setMainTab(tab);
    }
  ...
  return (
    <aside className="h-full w-full border-r border-base-content/10 flex flex-col bg-base-100/70 select-none overflow-hidden no-scrollbar">
      {/* Sidebar Top Header */}
      <div className="p-2.5 sm:p-4 border-b border-base-content/10 space-y-2.5 flex-shrink-0 bg-base-100/90 backdrop-blur-md">
        ...
        {/* WhatsApp Top Navigation Switch: Chats vs Status */}
        <div className="flex items-center gap-1 p-1 bg-base-200/80 rounded-2xl border border-base-content/5">
          <button
            type="button"
            onClick={() => {
              setMainTab("chats");
            ...
            className={`flex-1 py-1.5 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer ${
              mainTab === "chats"
                ? "bg-base-100 text-base-content shadow-xs ring-1 ring-base-content/5"
                : "text-base-content/60 hover:text-base-content"
            }`}
          >
            <MessagesSquare
              size={15}
              className={mainTab === "chats" ? "text-emerald-600 dark:text-emerald-400" : ""}
            />
            <span>Chats</span>
            {totalUnread > 0 && (
              <span className="badge badge-xs bg-emerald-500 text-white font-bold px-1.5">
                {totalUnread}
              </span>
            )}
          </button>
          ...
          >
            <CircleDashed
              size={15}
              className={mainTab === "status" ? "text-emerald-600 dark:text-emerald-400 animate-spin-slow" : ""}
            />
            <span>Status</span>
        ...
        {/* Search Input Bar & Filter Tabs (Visible only in Chats view) */}
        {mainTab === "chats" && (
          <>
      ...
      {/* Main View: Status Tab OR Chats Tab */}
      {mainTab === "status" ? (
        <StatusSidebar />
      ) : (
        <>
            ...
            <div className="py-2.5 px-3 border-b border-base-content/5 bg-base-100/40 flex-shrink-0">
              ...
              <div className="flex items-center gap-2.5 overflow-x-auto no-scrollbar py-0.5 px-1">
                {onlineContacts.map((contact) => {
                  const contactStatusGroup = otherUserStatuses.find(
                    (g) => g.user?._id?.toString() === contact._id?.toString()
                  );
                  const hasContactStatus = Boolean(
                    contactStatusGroup && contactStatusGroup.statuses.length > 0
                  );
                  const hasUnviewedContactStatus = Boolean(contactStatusGroup?.hasUnviewed);
                      ...
                      <div className="relative">
                        <div
                          onClick={(e) => {
                            if (hasContactStatus) {
                              e.stopPropagation();
                              openStory(contactStatusGroup, 0);
                            }
                          }}
                          className={`p-0.5 rounded-2xl transition-all ${
                            hasContactStatus
                              ? hasUnviewedContactStatus
                                ? "ring-2 ring-emerald-500 ring-offset-2 ring-offset-base-100 shadow-sm shadow-emerald-500/30 group-hover:scale-105"
                                : "ring-2 ring-base-content/30 ring-offset-1 ring-offset-base-100"
                              : "bg-gradient-to-tr from-emerald-500 to-teal-400 group-hover:shadow-md group-hover:shadow-emerald-500/30"
                          }`}
                          title={hasContactStatus ? `View ${contact.fullName}'s status` : undefined}
                        >
                          {contact.profilePic ? (
                            <img
          ...
          <div className="overflow-y-auto w-full py-1.5 px-2 space-y-1 flex-1 no-scrollbar pb-20 md:pb-4">
            {filteredUsers.map((user) => {
              ...
              const statusGroup = otherUserStatuses.find(
                (g) => g.user?._id?.toString() === user._id?.toString()
              );
              const hasActiveStatus = Boolean(
                statusGroup && statusGroup.statuses.length > 0
              );
              const hasUnviewedStatus = Boolean(statusGroup?.hasUnviewed);
                ...
                >
                  ...
                  <div
                    onClick={(e) => {
                      if (hasActiveStatus) {
                        e.stopPropagation();
                        openStory(statusGroup, 0);
                      }
                    }}
                    className={`relative flex-shrink-0 ${hasActiveStatus ? "cursor-pointer" : ""}`}
                    title={hasActiveStatus ? `View ${user.fullName}'s status` : undefined}
                  >
                    <div
                      className={`p-0.5 rounded-2xl transition-all ${
                        hasActiveStatus
